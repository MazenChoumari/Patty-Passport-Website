      ...
      return '<div style="border-top:2px solid #1b1a19">'
        ...
        + items.map(function (it) {
          ...
          return '<div style="border-right:2px solid #1b1a19;border-bottom:2px solid #1b1a19;background:#f7f3ec;padding:18px 17px 20px;display:flex;flex-direction:column;gap:9px;min-height:184px">'
            ...
            + (it.kind === "veg" || it.kind === "chicken" || it.kind === "beef" || it.kind === "fries" || it.kind === "salad" ? '<button type="button" data-combo-open="' + esc(it.name) + '" data-combo-kind="' + it.kind + '" data-combo-price="' + it.price + '" data-combo-country="' + esc(d.code) + '" data-combo-country-name="' + esc(d.name) + '" data-combo-route="' + esc(route.name) + '" style="flex:1;padding:9px 10px;background:transparent;color:#1b1a19;border:2px solid #1b1a19;font:800 10px/1 \'Archivo\',sans-serif;letter-spacing:.1em;text-transform:uppercase;cursor:pointer" data-hover="background:#f2b30c;border-color:#f2b30c">Make it a combo</button>' : "")
            + '</div>'
            + '</div>';
        }).join("") + '</div></div>';
    }).join("");
