...
(function () {
  ...
  var KIT = [
    ["JUNIOR PASSPORT", "3%", "2%", "206px", CREAM, INK, "-5deg", "10s", "0s", "118px", "Junior passport cover in a child's hands", "14px 14px 0 rgba(27,26,25,.3)", "images/kit/junior-passport.jpg"],
    ["ROUTE BOOKLET", "46%", "14%", "220px", "#fff", INK, "4deg", "12s", ".8s", "136px", "Open route booklet, one page per route", "-12px 14px 0 rgba(27,26,25,.28)", "images/kit/route-booklet.jpg"],
    ["FLAG STICKERS", "7%", "44%", "194px", BLU, "#fff", "6deg", "11s", ".4s", "114px", "Sheet of 21 flag stickers, half peeled", "12px 12px 0 rgba(27,26,25,.3)", "images/kit/flag-stickers.jpg"],
    ["EXPLORER TOTE", "50%", "56%", "186px", RED, "#fff", "-7deg", "13s", "1.2s", "108px", "Kids explorer tote bag with contents", "-12px 12px 0 rgba(27,26,25,.3)", "images/kit/explorer-tote.jpg"],
    ["POSTCARD HOME", "22%", "76%", "202px", "#e7e3dc", INK, "2deg", "9s", ".2s", "90px", "Stamped destination postcard on the table", "12px 12px 0 rgba(27,26,25,.25)", "images/kit/destination-postcard.jpg"]
  ].map(function (k, i) {
    return { label: k[0], x: k[1], y: k[2], w: k[3], bg: k[4], fg: k[5], r: k[6], dur: k[7], delay: k[8], h: k[9], note: k[10], shadow: k[11], img: k[12], slotId: "jr-kit-" + i };
  });
  ...
  function renderKit() {
    document.getElementById("jr-kit").innerHTML = KIT.map(function (k) {
      return '<div style="position:absolute;left:' + k.x + ';top:' + k.y + ';width:' + k.w + ';background:' + k.bg + ';color:' + k.fg + ';border:2px solid #1b1a19;box-shadow:' + k.shadow + ';transform:rotate(' + k.r + ');--r:' + k.r + ';animation:ppDrift ' + k.dur + ' ease-in-out ' + k.delay + ' infinite;padding:12px">'
        ...
        + (k.img
          ? '<img id="' + k.slotId + '" src="' + k.img + '" alt="' + esc(k.note) + '" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">'
